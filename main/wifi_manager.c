...
// // Handler mới cho điều khiển đèn
...
//             {.uri = "/light", .method = HTTP_GET, .handler = light_handler},
//             {.uri = "/setpomodoro", .method = HTTP_POST, .handler = setpomodoro_handler},
//             {.uri = "/setpomodoro", .method = HTTP_OPTIONS, .handler = setpomodoro_handler},
...
// Handler mới: Lấy cài đặt Pomodoro hiện tại
...
// Handler mới: Cài đặt thời gian Pomodoro
static esp_err_t setpomodoro_handler(httpd_req_t *req) {
    if (req->method == HTTP_OPTIONS) {
        httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
        httpd_resp_set_hdr(req, "Access-Control-Allow-Methods", "POST, OPTIONS");
